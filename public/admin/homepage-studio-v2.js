(()=>{
  ...
  function targetPreview(section,x){
    ...
    if(section==='brandRail')return `<div class="hs-mini-stage dark"><h4>Untoz Brands</h4><div class="hs-brand-chips">${(x.brandRail||[]).map(name=>`<span class="hs-brand-chip">${esc(name)}</span>`).join('')}</div></div>`;
    if(section==='productions')return `<div class="hs-mini-stage"><h4>Featured Productions</h4><div class="hs-mini-grid">${(x.productions||[]).slice(0,3).map(item=>`<div class="hs-mini-tile"><small>${esc(item.eyebrow||'')}</small><b>${esc(item.title||'')}</b></div>`).join('')}</div></div>`;
    if(section==='products')return `<div class="hs-mini-stage"><h4>Untoz Products</h4><div class="hs-mini-grid">${(x.products||[]).slice(0,3).map(item=>`<div class="hs-mini-tile"><small>${esc(item.eyebrow||'')}</small><b>${esc(item.title||'')}</b></div>`).join('')}</div></div>`;
