(()=>{
  const productions=[
    ['UNT OZ AWARDS 2026','LIVE EVENT','Dubai · 2026','The next edition of the Untoz Awards arrives in Dubai.'],
  ...
  function section(title,kicker,copy,html,id){
    ...
    s.innerHTML=`<div class="untoz-v2-rail-head"><div><div class="label">${kicker}</div><h2>${title}</h2></div>${copy?`<p>${copy}</p>`:''}</div>${html}`;return s;
  }
  function buildProductions(){
    if(document.querySelector('#productions-v2'))return;
    const about=document.querySelector('#about');if(!about)return;
    const html=`<div class="u-production-grid">${productions.map((p,i)=>`<article class="u-production"><span class="u-year">${p[2]}</span><span class="u-tag">${p[1]}</span><h3>${p[0]}</h3><p>${p[3]}</p></article>`).join('')}</div>`;
  ...
  function buildCalendar(){
    if(document.querySelector('#calendar-v2'))return;
    const about=document.querySelector('#about');if(!about)return;
    ...
    about.parentNode.insertBefore(section('What\'s coming.','UNT OZ CALENDAR','A simple view of upcoming broadcasts, events and moments worth putting in your calendar.',html,'calendar-v2'),about);
  }
  function tuneCopy(){
  ...
  function boot(){buildProductions();buildCalendar();tuneCopy();removeCorporateStats()}
